...
void create_newProcess(char *arguements[], char **argv);
...
void create_newProcess(char *arguements[], char **argv)
{
	...
	if (newProcess == 0)
	{
		...
		arg_len = _strlen(arguements[0]);
		prog_len = _strlen(argv[0]);

		if (full_path == NULL)
		{
			write(STDOUT_FILENO, argv[0], prog_len);
			write(STDOUT_FILENO, ": ", 2);
			write(STDOUT_FILENO, "1", 1);
			write(STDOUT_FILENO, ": ", 2);
			write(STDOUT_FILENO, arguements[0], arg_len);
...
/**
  ...
  * Return: 0 on success
  */
int main(int argc __attribute__((unused)), char *argv[], char **env_command)
{
	...
	while (1 && interactive)
	{
		...
		/**Create a fork */
		create_newProcess(arguements, argv);
		argCount = 0;
	}
	free(buffer);
	return (0);
